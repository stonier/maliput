...
class kdtree {
 ...
 private:
  ...
  struct node_cmp {
    ...
    bool operator()(const node& n1, const node& n2) const { return n1.point_.get(index_) < n2.point_.get(index_); }
    size_t index_;
  };
...
template <typename coordinate_type, size_t dimensions>
struct less<maliput::math::point<coordinate_type, dimensions>> {
  bool operator()(const maliput::math::point<coordinate_type, dimensions>& lhs,
                  const maliput::math::point<coordinate_type, dimensions>& rhs) const {
